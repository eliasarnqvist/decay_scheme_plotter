...
def parse_ensdf_levels_gammas(path, nucid):
    ...
    for i, line in enumerate(lines):
        ...
        # Make sure the nucid is the same
        if line[:5].upper() != nucid:
            continue
    ...
    all_levels = sorted(levels.keys(), reverse=True)

    # print(levels)
    # print(all_levels)
    return levels, all_levels
...
def plot_decay_scheme(levels, all_levels, parent="??", daughter="??"):
    plt.close('all')
    inch_to_mm = 25.4
    color = plt.cm.tab10
    ...
    for E in all_levels:
        ...
        for gamma in level["G"]:
            ...
            ax.annotate(None, xy=(x_pos, E-Eg), xytext=(x_pos, E), 
                        ...
                        zorder=3)
            ax.text(x_pos, E, f"{Eg:.2f} keV, {Ig}\%",
                    rotation=70, rotation_mode="anchor", ha="left", va="bottom", fontsize=8,
                    bbox=dict(boxstyle="square,pad=0.01", fc="white", ec="none"),
                    zorder=4)
            
            x_pos += x_step

        x_pos_decay_arrow_left = - x_pad_left * x_max + 0.02
    ...
    y_pos_arrow = (1 + y_pad_over - 0.1) * energy_span
    ax.annotate(None, xy=(x_pos_decay_arrow_left, 0.0), xytext=(x_pos_decay_arrow_left, y_pos_arrow), 
                arrowprops=dict(arrowstyle="-", lw=1.0, shrinkA=0, shrinkB=0),
                zorder=4)
    ax.annotate(None, xy=(x_pos_decay_arrow_right, y_pos_arrow), xytext=(x_pos_decay_arrow_left, y_pos_arrow), 
                arrowprops=dict(arrowstyle="-", lw=2.0, shrinkA=0, shrinkB=0),
                zorder=4)

    ax.text(0.5, 0.0-0.03*energy_span, daughter, ha="center", va="top", fontsize=10, zorder=4)
    ax.text((x_pos_decay_arrow_left+x_pos_decay_arrow_right)/2, y_pos_arrow-0.03*energy_span, parent, ha="center", va="top", fontsize=10, zorder=4)

    plt.show()


levels, all_levels = parse_ensdf_levels_gammas("ensdf_files/140la.txt", nucid="140CE")
levels, all_levels = limit_intensity(levels, I_min=10)

print(levels)

plot_decay_scheme(levels, all_levels, 
                  parent=r"$\mathrm{\prescript{140}{57}{La}}$", 
                  daughter=r"$\mathrm{\prescript{140}{56}{Ce}}$")
